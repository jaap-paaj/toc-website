---
title: 'AI proof of concept: the difference between knowing and deciding'
date: '2026-04-03'
intro: >-
...
Too alive to bury, too weak to scale. We call them [Zombie Pilots](/nl/blog/zombie-pilots).
...
At the end of those two weeks, there is a working solution. Not a report, not a presentation, something that runs. Plus the data to decide: do we continue, do we adjust, or do we stop? Only once that evidence exists is [scaling justified](/nl/blog/proof-before-scale).
...
Ready to get started? Begin with an [AI Workshop](/nl/educate) to give your team the foundations. Or start an [Innovation Sprint](/nl/innovate) to test in two weeks where AI genuinely creates value for your organisation.

---

## Frequently Asked Questions about AI proof of concepts

**What is an AI proof of concept?**
An AI proof of concept is a short, defined experiment to test whether artificial intelligence adds value to a specific business process. The goal is to produce decision data: does this work, and if so, is it worth the investment to scale? A good proof of concept takes two to four weeks and produces a working prototype tested by real users.

**What does an AI proof of concept cost?**
At The Only Constant, a proof of concept engagement costs around €15,000 for two weeks of intensive work. This includes sharpening the problem, building a working prototype, testing with real users, and delivering decision data. For smaller questions, we start with an AI Opportunity Scan (from €2,500) to first determine whether a proof of concept is the right next step.

**How long does an AI proof of concept take?**
Two to four weeks, strictly defined. The time pressure is intentional: it forces you to make choices and prevents the project from growing into an endless investigation. In week one you build; in weeks two to four you test with real users and measure. At the end there is a working prototype plus the data to make a decision.

**When is an AI proof of concept worthwhile?**
A proof of concept is worthwhile when you have a specific process where you suspect AI can add value, but you still lack the evidence. Prerequisites: a clearly defined problem, real users available to test, and someone in the organisation with the authority to make a decision at the end (continue, adjust, or stop).

**What is the difference between an AI pilot and an AI proof of concept?**
A pilot often runs for months and aims to prove the technology works. A proof of concept runs for weeks and aims to produce decision data. The distinction is crucial: "the technology works" says nothing about whether it delivers value for your organisation. A proof of concept tests the value, not the technology.

**What happens after an AI proof of concept?**
Four possible outcomes: scale (it works, we proceed), adjust and retest (the direction is right, the execution needs work), change direction (what we learned points to a better use case), or stop (the value is not there, and that is also a valid conclusion). At The Only Constant we document all four outcomes so the organisation learns as much as possible from the investment.
